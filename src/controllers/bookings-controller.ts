...
export async function createBooking(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.body.roomId || req.body.roomId != number) throw badRequestError();
      const userId = req.userId as number;
      await hotelsService.verification(userId);
      const roomId: number = req.body.roomId;
      const bookingId: number = await bookingsService.bookRoom(userId, roomId);
...
export async function alterBooking(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.params.bookingId || !req.body.roomId) throw badRequestError();
      const userId = req.userId as number;
